...
\begin{equation}
\Delta P_\text{min} = \frac{8 E I \cdot h_\text{Aufbiegung}}{W \cdot L^4}
\end{equation}
Mit den Nominaldaten ($t = 0{,}354$\,mm, $E = 200$\,GPa) ergibt die Formel $\approx 370$\,Pa. Praktisch gemessene Schwellendrücke für diese Zungenklasse liegen bei $\approx 220$\,Pa. Die Abweichung erklärt sich durch die \textbf{Zungenprofilierung}: Basszungen werden am freien Ende dünner gefeilt, um die Masse für die niedrige Frequenz zu optimieren. Die effektive Dicke liegt daher unter der am Einspannende gemessenen 0{,}354\,mm. Für die folgenden Berechnungen wird der Erfahrungswert $\Delta P_\text{min} \approx 224$\,Pa verwendet.
...
\begin{keybox}
\textbf{Grenzdruck:} Der Spalt schließt sich bei $\Delta p_\text{crit} = 8EIh_\text{max}/(WL^4)$. Mit der effektiven (profilierten) Dicke ergibt sich $\approx 221$\,Pa $\approx 2{,}2$\,mbar. Das ist \textbf{identisch} mit dem Schwellendruck $\Delta p_\text{min} = 224$\,Pa -- und das ist kein Zufall: Beide Formeln berechnen denselben physikalischen Effekt (Gleichlast, die die Aufbiegung kompensiert).
\end{keybox}
...
\vspace{4mm}
{\small\textbf{Querverweise:} Dok.\,0003 (Impedanzvergleich), Dok.\,0004 (Zwei-Filter-Modell), Dok.\,0005 (Ansprache-Indikator), Dok.\,0006 (Zeichenerklärung), Dok.\,0007 (Diskant-Kammerfrequenzen), Dok.\,0008 (Klangveränderung). Alle Berechnungen verifiziert in \href{berechnungen_verifikation.py}{berechnungen\_verifikation.py}.}
